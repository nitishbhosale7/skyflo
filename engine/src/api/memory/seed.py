...
async def seed_memory_stores() -> None:
    ...
    """
    if not await _is_memory_available():
        logger.info("Memory tables not present yet; skipping seed")
        return
...
async def _is_memory_available() -> bool:
    """Return False if memory tables are not migrated yet (safe startup guard)."""
    try:
        from tortoise import connections

        conn = connections.get("default")
        rows = await conn.execute_query_dict(
            "SELECT to_regclass('public.memory_stores') IS NOT NULL AS exists"
        )
